...
- [x] **第三階段：分析功能擴充**：見文件最後「第三階段規劃」（2026-09-17 全部 15 項完成）
...
| 期貨三大法人 | TAIFEX `cht/3/futContractsDateDown`（POST CSV，Big5；OpenAPI 只有最新一天） | 可，一次一個月 | 台指期多空交易與未平倉口數與金額 |
...
10. ✅ **借券賣出餘額**：TWT93U 每日收集＋回補 → 個股詳情趨勢、籌碼指標與提示詞（借券賣出增加是潛在賣壓）。與外資持股共用回補流程（`src/ownership.py`），每日收集另補近兩週缺漏；TWT93U 兩段欄位名稱重複，借券欄位依位置取並檢查格式
11. ✅ **除權息與事件行事曆**：TWT48U 除權息預告＋月營收公布期限（每月 10 日）＋持股相關事件；新頁面「行事曆」，持股與觀察名單優先顯示（持股附預估現金股利）；報告列出近期事件。上櫃用櫃買 OpenAPI `tpex_exright_prepost`；另加入 AI 預測檢驗日
12. ✅ **K 線支撐壓力＋成交量密集區**：近期轉折高低點聚類成支撐壓力、價量分布（Volume Profile）；畫在 K 線圖上並放進個股提示詞
13. ✅ **季度 EPS／毛利率／ROE**：改用公開資訊觀測站舊站 `ajax_t163sb04`／`t163sb05`（全市場、可指定年季）回補 8 季，每日收集重抓最新一季；六種行業報表欄位以關鍵字比對；累計值相減得單季；個股詳情季度圖、選股獲利條件、提示詞。實測台積電 2024Q4 單季 EPS 14.45 元與公告一致
14. ✅ **本益比河流圖**：回補歷史 BWIBBU_d；以「股價／本益比」推算 EPS，畫出歷史本益比分位數（10／25／50／75／90%）對應的價格帶；選股加「本益比歷史百分位 ≤」。驗證：台積電以股價÷本益比推算 EPS 86.26 元，與季報近四季 EPS 86.28 元一致；只支援上市股
15. ✅ **期貨三大法人未平倉**：期交所 CSV 下載（可指定區間，按月回補 12 個月）→ `futures_institutional` 表；每日收集抓近兩週覆寫；市場總覽三大法人淨未平倉卡片與 60 日趨勢；大盤提示詞加入淨部位與 1／5／20 日變化
