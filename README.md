...
### 資料收集
- **TWSE / TPEx 官方資料**：收盤價量、三大法人買賣超、融資融券、本益比／殖利率／淨值比、月營收、加權指數、
  外資持股比例、借券賣出餘額、除權息預告
- **公開資訊觀測站**：月營收歷史、季度損益與資產負債（EPS、毛利率、營益率、ROE）
- **集保中心**：股權分散表（千張大戶、散戶持股）；**期交所**：台指期三大法人未平倉
- **FinMind API**：K 線歷史與交叉驗證
- **鉅亨網 API + Google News RSS**：當日新聞與觀察名單個股的延伸新聞
- 資料存在本機 SQLite；上市股歷史可一次補收集，每日收集時也會自動補上近兩週漏掉的交易日
- 每日自動收集（Windows 工作排程器，時間可自選，預設 20:00），錯過時間會在下次開機後補做；
  收集完要不要用 Codex 分析新聞、持股、觀察名單可以各自開關，避免額度用光

### AI 分析（Codex CLI）
- 使用這台電腦已登入的 Codex CLI 與帳號額度，**不需要填 API Key**
- **新聞深度分析**：先擷取新聞內文（鉅亨網 API 內建全文；其他來源優先用 Firecrawl、退回本機 Playwright），
  逐篇摘要後彙整挑出當日焦點個股（前 10／20／30／50 檔可選）；收集完可自動觸發
- **個股分析**：依程式算好的技術指標、籌碼延伸指標、基本面、新聞分析技術面／籌碼面／1～2 週展望（每項 800 字內）；
  持有的股票會附上成本與損益，列出續抱／減碼／停損的條件式觀察點，不直接下買賣指令
- **大盤籌碼分析**：上市櫃個股法人加總、加權指數、市場溫度計、期貨未平倉、新聞重點，總計 1000 字內
- **AI 預測追蹤**：自動解析每次個股分析的方向與支撐壓力，5／10 個交易日後對照實際走勢統計命中率
...
- **觀察名單**（選股工具內）：可建立多個名單組合，顯示價格與訊號，區分「今日新出現」與「持續中」
- **訊號回測**：訊號出現後 5／20 個交易日的報酬分布、勝率與相對大盤的超額報酬
- **市場總覽**：市場溫度計（創新高／新低家數、漲跌家數比）、期貨三大法人未平倉、產業熱力圖
- **個股詳情**：K 線支撐壓力與成交量密集區、相對大盤強弱、月營收與季度財報、本益比河流圖、千張大戶、外資持股與借券賣出
- **條件提醒**：價格、漲跌幅、持股損益、觀察名單訊號，收集完成後跳 Windows 通知
- **行事曆**：除權息（持股附預估現金股利）、月營收公布期限、AI 預測檢驗日

### 持股與報告
- **歷史查詢**：依期間與關鍵字查過去的新聞、新聞焦點（含上榜次數統計）、個股與大盤分析、AI 預測與檢驗結果
- **我的持股**：記錄每筆買進與賣出（含手續費、交易稅、買賣理由），計算平均成本、未實現與已實現損益，可逐檔用 Codex 分析，賣出後可請 Codex 覆盤
...
- [x] 我的持股
- [x] Windows 安裝程式與自動更新
- [x] 第三階段：預測追蹤、產業熱力圖、條件提醒、大戶持股、交易日誌、市場溫度計、財報、本益比河流圖、期貨法人等（見 PLAN.md）
